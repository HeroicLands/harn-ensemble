...
- 658c3de: **This tree's folders are notes, and a note names one by address.**
  ...
  **Verified by rebuilding the tree either side of the change.** Every one of the
  ...
  subject of #45.
- eaefcb6: **The top-level `traits:` block is gone from all 2,512 being notes; every field
...
- 94ff792: Author the HM3 system fields under `hm3.system`, and stop stating a being's
  species twice.
  ...
  **Verification.** The packs cannot be the evidence here: this tree still
  ...
  Closes #51
- 728d41e: Author every SoHL system field under `sohl.system`, at the path the compiled
...
- e38a62f: Drop the `journals` pack, which had no source and failed every build.
  ...
  **What this does not fix.** The Actor passes still fail — they require
  ...
  Refs #21
- 24997f7: **One CI shape across the four content repositories.** `.github/workflows/lint.yml`
  ...
  Four of five checks pass; `lint:markdown` is red on 170 pre-existing findings, none introduced here. `build:noci` did not run `lint` at all before this, so the release path checked nothing.
...
- 89ace9c: Publish the content tree at `https://www.heroiclands.org/harnensemble/`, not just
  the homepage.
...
- a396d76: Author the template-priority number as `data.templatePriority` rather than
